...
#In and outputs are stored in this list, not relavant per sss
...
#Check study vars name FILE is represnting codesheet
FILE <- FILE[Outcome %in% c("COVID19DX"),]
...
for(i in files){
  ...
  #Create PERSONS_OF_INTEREST1 variable at the beginning of the script first containing the study cohort subjects
  TEMP <- IMPORT_PATTERN(
  ...
  files <- list.files(path_dir, pattern = scheme[j])
      ...
      #i=files[1]
          for(i in files){
              ...
              #Remove records before first known Covid infection also add this here if only covid 
              #before <- nrow(TEMP)
              #TEMP <- TEMP[Date > as.Date("20200101", "%Y%m%d") & Date < end_study_date,]
              #after <- nrow(TEMP)
...
#If you want you can combine the diagnosis and MO/SO info in 1 file. Change the COVID19DX and COVID19DX2 studyvars accordingly
