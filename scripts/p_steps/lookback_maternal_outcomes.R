...
output_folders<-list(g_output_mat_out_pan_neg, g_output_mat_out_hist,g_output_mat_out_pan_pos)
...
for(i in 1:length(exposure_folders)){
  ...
  for (j in 1:length(covariate_tables)){
    ...
    results<-lookforward_or_during_test(expos_data = exposure_data, preg_data = df_preg_dates, lookforward =90)
    #print(results)
    #print(j)
