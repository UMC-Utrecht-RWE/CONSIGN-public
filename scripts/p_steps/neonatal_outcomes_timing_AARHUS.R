...
cohort_covariate_folders<-c( output_neonates_hist)

# we need the cov_date for each case and control
cohort_child_data<-c(paste0(historical_neonate_folder,"historical_neonates.csv"))


output_folders<-c( output_neonates_hist)
cohort_names<-c("hist_neonates_outcomes.csv")
