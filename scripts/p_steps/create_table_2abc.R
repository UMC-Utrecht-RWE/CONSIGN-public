...
control_data<-fread(paste0(matched_folder,"matches_pregnant_cov_neg.csv"))
control_data$gest_age_cov<-control_data$cov_date-control_data$pregnancy_start_date
control_colnames<-colnames(control_data)
new_names<-c(control_colnames[1:42],"empty_trim", "empty_date",control_colnames[45:ncol(control_data)])

colnames(control_data)<-new_names
...
baseline_output<-as.data.frame(cbind(baseline_rownames,total_cases, severe_cases, nonsevere_cases, total_controls))

colnames(baseline_output)<-c("output", "all_cases", "severe_cases", "nonsevere_cases", "pregnant matched controls")

baseline_covid_month<-as.data.frame(cbind(levels_calendar, as.numeric(covid_month_cases), as.numeric(covid_month_severe), as.numeric(covid_month_nonsevere), as.numeric(covid_month_control)))
