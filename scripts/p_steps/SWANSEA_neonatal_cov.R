...
for(i in 1:length(neonate_cohort_folders)){
  ...
  #################################################################################
  # LBW
  # only survey observations
  # so_source_table= "MDR" AND so_source_column == "peso"  AND so_source_value= <2500
  
  
  #so_source_table='mids_m' AND so_source_column='SERVICE_USER_WEIGHT_GRAMS' AND AND so_source_value= <2500
  
  LBW_SO_ID<-SURV_OB$person_id[((SURV_OB$so_source_table=="mids_m")&(SURV_OB$so_source_column=="SERVICE_USER_WEIGHT_GRAMS")&(SURV_OB$so_source_value<=2500))]
  LBW_SO_Date<-SURV_OB$so_date[((SURV_OB$so_source_table=="mids_m")&(SURV_OB$so_source_column=="SERVICE_USER_WEIGHT_GRAMS")&(SURV_OB$so_source_value<=2500))]
...
# low apgar
  ...
  # so_source_column="BIRTH_APGAR_SCORE" and AND so_source_value= "0" or "1" or "2" or "3"
  
  APGARLOW_SO_ID<-SURV_OB$person_id[((SURV_OB$so_source_column=="BIRTH_APGAR_SCORE")&(SURV_OB$so_source_value%in%c(0,1,2,3,4,5,6)))]
  APGARLOW_SO_Date<-SURV_OB$so_date[((SURV_OB$so_source_column=="BIRTH_APGAR_SCORE")&(SURV_OB$so_source_value%in%c(0,1,2,3,4,5,6)))]
