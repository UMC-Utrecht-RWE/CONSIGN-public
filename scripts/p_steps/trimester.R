# ARS TOSCANA interim CONSIGN trimester function

# load D3_included_pregnancies  
load(paste0(dirtemp,"D3_pregnancy_final.RData"))



### Creating trimesters
#Trimester 1: from Last Menstrual Period (LMP) to day < 98 after LMP; or end of pregnancy, whichever earlier
D3_preg_trim <- D3_pregnancy_final[, trim1_start := pregnancy_start_date]
...
D3_preg_trim_2 = melt(D3_preg_trim, 
                      id.vars = c("person_id", "pers_group_id", 
                                  ...
                                  "at", "end_preg_in_2021", "age", "age_category"),
...
D3_pregnancy_trimester <- D3_preg_trim_3

# create variable "status"
D3_pregnancy_trimester <- D3_pregnancy_trimester[, covid_in_preg := max(covid_in_trim), by = "pers_group_id"]
D3_pregnancy_trimester <- D3_pregnancy_trimester[covid_in_trim == 1, trimester_covid := as.integer(trim)]

D3_pregnancy_trimester <- D3_pregnancy_trimester[is.na(trimester_covid), trimester_covid := 0]

#D3_pregnancy_trimester <- D3_pregnancy_trimester[, trimester_covid := as.integer(trimester_covid)]
D3_pregnancy_trimester <- D3_pregnancy_trimester[, trimester_covid := max(trimester_covid), by = "pers_group_id"]

D3_pregnancy_trimester <- D3_pregnancy_trimester[covid_in_trim == 1, status := 1]
D3_pregnancy_trimester <- D3_pregnancy_trimester[covid_in_preg == 1 & covid_in_trim == 0 & (trimester_covid <  as.integer(trim)), status := 2]
D3_pregnancy_trimester <- D3_pregnancy_trimester[is.na(status) & prior_to_COVID == 0, status := 0]
D3_pregnancy_trimester <- D3_pregnancy_trimester[prior_to_COVID == 1, status := NA]

#View(D3_pregnancy_trimester[, .(covid_in_preg, pers_group_id, trim, covid_in_trim, status,trimester_covid )][order(covid_in_preg, pers_group_id, trim )])
D3_pregnancy_trimester <- D3_pregnancy_trimester[, -c("covid_in_preg", "trimester_covid" )]
save(D3_pregnancy_trimester, file=paste0(dirtemp,"D3_pregnancy_trimester.RData"))

rm(D3_pregnancy_trimester, D3_preg_trim_3, D3_preg_trim_2, D3_preg_trim,
   D3_pregnancy_final, MFC_covid_in_trim, covid_registry, SURVEY_ID_COVID,
   D3_PERSONS)