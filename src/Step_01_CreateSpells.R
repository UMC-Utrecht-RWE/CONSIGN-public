...
OBSERVATION_PERIODS <- fread(paste0(, "OBSERVATION_PERIODS.csv"))
...
step1_unique<-length(unique(OBSERVATION_PERIODS$person_id))
  ...
  saveRDS(OBSERVATION_PERIODS1, file = paste0(,"ALL_OBS_SPELLS.rds"))
  ...
  if(exists("FlowChartOverlap")){
    saveRDS(FlowChartOverlap, file = paste0(output_dir,"SUBPOP_FlowChartOverlap.rds"))
    rm(FlowChartOverlap)
  }
