...
OBSERVATION_PERIODS <- fread(paste0(pathCDM, "OBSERVATION_PERIODS.csv"))
...
actual_tables_preselect$EVENTS<-list.files(pathCDM, pattern="^EVENTS")
actual_tables_preselect$MEDICAL_OBSERVATIONS<-list.files(pathCDM, pattern="^MEDICAL_OBSERVATIONS")
actual_tables_preselect$SURVEY_OBSERVATIONS<-list.files(pathCDM, pattern="^SURVEY_OBSERVATIONS")
actual_tables_preselect$MEDICINES<-list.files(pathCDM, pattern="^MEDICINES")
actual_tables_preselect$VACCINES<-list.files(pathCDM, pattern="^VACCINES")
actual_tables_preselect$SURVEY_ID<-list.files(pathCDM, pattern="^SURVEY_ID")
actual_tables_preselect$EUROCAT<-list.files(pathCDM, pattern="^EUROCAT")
actual_tables_preselect$PERSONS<-list.files(pathCDM, pattern="^PERSONS")

all_actual_tables<-list.files(pathCDM, pattern = "\\.csv$")
...
if(length(actual_tables_preselect$PERSONS)>1){
  PERSONS<-lapply(paste0(pathCDM,actual_tables_preselect$PERSONS), fread)
  PERSONS<-do.call(rbind,PERSONS)
  PERSONS<-as.data.table(PERSONS)

}else {
  PERSONS<-fread(paste0(pathCDM,actual_tables_preselect$PERSONS))
...
for(i in 1:length(tables_vec_all)){
  ...
  mytable<-fread(paste0(pathCDM,tablename))
