...
events_tables<-IMPORT_PATTERN(pat="EVENTS", dir = )
events_tables$event_code<-	"B34.2"
events_tables$event_record_vocabulary<-"ICD10"
head(events_tables)
codesheet<-fread(paste0(projectDir,"/ALL_full_codelist.csv"))
...
CreateConceptDatasets <- function(codesheet=codesheet2,file=events_tables, c.voc="coding_system", c.concept="full_name", c.codes="code", c.startwith = NULL,
                                  f.code="event_code", f.voc="event_record_vocabulary", path = , method = "loop", group = T, f.name = NULL, db = NULL ){
  # browser()
...
covid_DX<-readRDS(paste0( , "I_COVID19DX_COV.rds"))
